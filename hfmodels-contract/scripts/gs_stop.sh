...
F_SERVICE_NAME=hfmodel_guardian_service
...
# kill the service
if kill -SIGTERM ${F_PID} > /dev/null
then
    sleep 1
fi

# clean up the PID file if we were successful
if ps -p ${F_PID} > /dev/null
then
    yell failed to terminate process ${F_PID}
else
